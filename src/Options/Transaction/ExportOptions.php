...
final class ExportOptions extends OptionsAbstract
{
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        ...
        $resolver->define('currency')
            ->allowedTypes('string')
            ->info("Specify the transaction currency to export. Allowed values are: in kobo if currency is NGN, pesewas, if currency is GHS, and cents, if currency is ZAR");
        ...
        $resolver->define('settled')
            ->allowedTypes('bool')
            ->info("Set to true to export only settled transactions. false for pending transactions. Leave undefined to export all transactions");
            
        $resolver->define('settlement')
            ->allowedTypes('int')
            ->info("An ID for the settlement whose transactions we should export");
            
        $resolver->define('payment_page')
            ->allowedTypes('int')
            ->info("Specify a payment page's id to export only transactions conducted on said page");
    }
}
